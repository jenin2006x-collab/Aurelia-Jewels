...
async function handle(req,res,pathName){
  ...
  if(pathName==='/api/orders'&&method==='POST'){const u=auth(req,res);if(!u)return;const d=await body(req);let items;try{items=sanitizeItems(d.items||cartFor(u));}catch(e){return fail(res,400,e.message)}if(!items.length)return fail(res,400,'Your shopping bag is empty');const totals=calc(items,d.coupon,d.deliveryCost);const order={id:'AJ-'+crypto.randomBytes(4).toString('hex').toUpperCase(),createdAt:new Date().toISOString(),status:'Placed',userId:u.id,customer:{name:u.name,email:u.email,phone:u.phone},address:d.address||null,deliveryMethod:d.deliveryMethod||'Standard Delivery',paymentMethod:d.paymentMethod||'Cash on Delivery',items,totals,coupon:d.coupon||null,tracking:[{status:'Placed',at:new Date().toISOString()}]};db().orders.unshift(order);db().carts[u.id]=[];dbmod.save();return json(res,201,{order})}
  if(pathName==='/api/orders'&&method==='GET'){const u=auth(req,res);return u?ok(res,{orders:db().orders.filter(o=>o.userId===u.id)}):null}
  const om=pathName.match(/^\/api\/orders\/([^/]+)$/);if(om&&method==='GET'){const u=auth(req,res);if(!u)return;const o=db().orders.find(x=>x.id===om[1]&&x.userId===u.id);return o?ok(res,{order:o}):fail(res,404,'Order not found')}
