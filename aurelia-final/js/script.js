(function(){
  ...
  function setFiltersOpen(open) {
    ...
    if (filtersOpen) {
      ...
      catalogLayout?.classList.add('filters-open');
      ...
      catalogLayout?.classList.remove('filters-open');
  ...
  window.setAureliaFiltersOpen = setFiltersOpen;
  ...
  function catalog(){
    ...
    if (catParam) {
      ...
      // Auto-check the category in sidebar if present (does NOT open filter UI)
      document.querySelectorAll('.category-filter').forEach(cb => {
        const val = cb.value.toLowerCase();
        const param = catParam.toLowerCase();
        if (val === param) {
          cb.checked = true;
        } else if (param === 'necklace sets' && val === 'necklace sets') {
          cb.checked = true;
        } else if (param === 'necklaces' && val === 'necklaces') {
          cb.checked = true;
        } else if ((param === 'chains' || param === 'cross chains' || param === 'cross chain') && val === 'chains') {
          cb.checked = true;
        } else if ((param === 'rings' || param === 'hug rings' || param === 'hug ring') && val === 'rings') {
          cb.checked = true;
        }
      });
    ...
    const apply = () => {
      ...
      const maxPrice = Number(document.getElementById('priceRange')?.value || 60000);

      arr = arr.filter(p => {
        if (selectedCats.length && !selectedCats.some(c => p.category.toLowerCase() === c)) return false;
    ...
    if (!catalogEventsBound) {
      ...
      document.getElementById('clearFilters')?.addEventListener('click', () => {
        ...
        const eyeEl = document.getElementById('catalogEyebrow');
        if (eyeEl) eyeEl.textContent = 'AURELIA COLLECTIONS';
      ...
      // Filter toggle events - explicitly controlled by user click
      document.getElementById('openFilters')?.addEventListener('click', (e) => {
        e.preventDefault();
        setFiltersOpen(!filtersOpen);
      });

      document.getElementById('closeFilters')?.addEventListener('click', (e) => {
        e.preventDefault();
      ...
      window.addEventListener('resize', () => {
        ...
        } else if (filtersOpen && window.innerWidth <= 900) {
          document.getElementById('drawerOverlay')?.classList.add('open');
        }
      });

      // Ensure panel is strictly closed on initialization
      setFiltersOpen(false);
    }
  ...
  function bindQuickView(){
    ...
    document.addEventListener('click',e=>{const b=e.target.closest('.quick-btn');if(!b)return;e.preventDefault();const p=AureliaData.getProductById(b.dataset.quickId);if(!p)return;id=p.id;qty=1;const map={qvCategory:p.category,qvName:p.name,qvPrice:UI.formatPrice(p.price),qvOldPrice:UI.formatPrice(p.oldPrice),qvDiscount:p.discount+'% OFF',qvDesc:p.desc||p.description};Object.entries(map).forEach(([k,v])=>{const el=document.getElementById(k);if(!el)return;el.textContent=v});const embTitle=document.getElementById('qvEmblemTitle');if(embTitle)embTitle.textContent=p.name;const embSub=document.getElementById('qvEmblemSub');if(embSub)embSub.textContent=(p.metal||'22K Gold')+' • '+(p.subcategory||'Hallmarked');const qvImage=document.getElementById('qvImage');if(qvImage){qvImage.src=p.image||p.img||(Array.isArray(p.images)&&p.images[0])||UI.fallbackImg(p.category);qvImage.alt=p.name;}document.getElementById('qvQty').textContent='1';document.getElementById('quickViewModal')?.classList.add('open')});
  }
  function hero(){
