(function(){
  const FALLBACK_MAP = {
    ...
    Necklace: '/images/categories/necklaces.jpg',
    Earrings: '/images/categories/earrings.jpg',
    Rings: '/images/categories/rings.jpg',
    'Hug Rings': '/images/categories/hug_rings.jpg',
    Chains: '/images/categories/chains.jpg',
    'Cross Chains': '/images/categories/cross_chains.jpg',
  ...
  function renderDrawer(){
    ...
    if(w){
      ...
      w.innerHTML=a.length?a.map(i=>`<div class="drawer-item"><img src="${i.img}" alt="${i.name}" onerror="this.onerror=null;this.src='${fallbackImg(i.category)}';"><div><b>${i.name}</b><small>${fmt(i.price)}</small><button class="text-btn" onclick="Cart.add(${i.id});UI.toast('Added to bag')">Add to bag</button></div></div>`).join(''):'<div class="empty-state">No saved favourites yet.</div>';
    }
  }
  window.UI={
    formatPrice:fmt,
    ...
    closeDrawers(){document.querySelectorAll('.drawer,.drawer-overlay').forEach(e=>e.classList.remove('open'))}
  };
